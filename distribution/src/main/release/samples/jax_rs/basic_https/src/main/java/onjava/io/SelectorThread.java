...
import onjava.ssl.SSLChannelManager;
  ...
  public void registerChannelNow(SelectableChannel channel, 
    ...
    try {
      if (channel.isRegistered()) {
        ...
        sk.interestOps(interestSet);
        Object previousAttach = sk.attach(handlerInfo);
        assert previousAttach != null;
      } else {  
        channel.configureBlocking(false);
        SelectionKey registeredKey = channel.register(selector, interestSet, handlerInfo);   // qui  registra serverSocketchannel presso selector fornendo interestSet e l' handler [handlerInfo] che e' istanza Acceptor
        System.out.println(channel + " registrato in presso selector");
      }  
    } catch (Exception e) {
...
//  	NDC.push("S" + nameParts[nameParts.length-1]);  	
    ...
    while (true) {   
      ...
      int selectedKeys = 0;
      try {
        selectedKeys = selector.select();//selectedKeys e' il numero di keys=>channels correntemente registrati con selector che sono pronti per I/O operations
                                         //thread si blocca qui in attesa che uno o piu channels registrati abbiano qualcosa da fare...
      ...
      // any active event.
      while (it.hasNext()) {
        SelectionKey currentSelectionKey = (SelectionKey)it.next();
        SelectableChannel currentSelectionKeyChannel=   currentSelectionKey.channel();
        SelectorHandler handler =  (SelectorHandler) currentSelectionKey.attachment(); //Quando un channel e' registrato, l handler e' aggiunto come attachment alla SelectionKey!
        ...
        try {
          ...
          //###########   1/2  Quale operazione e' pronta per questo channel?   #################
          int readyOps = currentSelectionKey.readyOps();       
          // Disable the interest for the operation that is ready.
          // This prevents the same event from being raised multiple 
          // times.
          currentSelectionKey.interestOps(currentSelectionKey.interestOps() & ~readyOps);
          ...
          //###########   2/2  Invoca l handler  #################
          
          /*A - Channel pronto ad accettare nuove connessioni*/
           if (currentSelectionKey.isAcceptable()) {
            // A connection is ready to be completed
            ((AcceptSelectorHandler)handler).handleAccept();  //Fa 3 code:      1)Riattiva interest OP_ACCEPT del channel cosi che possa accettare nuove richeste
                                                                              //2)Ottiene un SocketChannel dal ServerSocketChannel
                                                                              //3)Manda il SocketChannel e Acceptor sopra a Server.socketConnected()  dove un PacketChannel viene istanziato
                                                                              //      e Server e' registrato come PacketChannelListener
            
            
          /*B - Channel pronto ad completare una connessione esistente*/  
          } else if (currentSelectionKey.isConnectable()) {
            // A connection is ready to be accepted            
            ((ConnectorSelectorHandler)handler).handleConnect();            
            
          } else {
          /*C - Connessione accettata e completa: dati possono essere scambiati!!*/
            ReadWriteSelectorHandler rwHandler = (ReadWriteSelectorHandler)handler; 
            // Readable or writable              
            if (currentSelectionKey.isReadable()) {                
              // It is possible to read
              rwHandler.selectorThreadHandleRead();              
            }
            ...
            if (currentSelectionKey.isValid() && currentSelectionKey.isWritable()) {
              // It is read to write
              rwHandler.selectorThreadHandleWrite();                              
            }
          }
        } catch (Throwable t) {
